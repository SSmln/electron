...
export const contentProjectSlice = createSlice({
  ...
  reducers: {
      ...
      switch (action.payload.sender) {
        case "sideItem":
          if (
            ...
            state.clickedIdx = state.tabComponents.findIndex(
              (e) => e.name === action.payload.name
            );
            state.clickedName = action.payload.name;
          }
    ...
    onProjectRemoveClick: (state) => {
      const currentIdx = state.tabComponents.findIndex(
        (e) => e.name === state.clickedName
      );
      state.tabComponents = state.tabComponents.filter(
        (obj) => obj.name !== state.clickedName
      );
      state.projectComponents = state.projectComponents.filter(
        (obj) => obj.name !== state.clickedName
      );
      state.tabComponents.length === 0
        ? ((state.clickedIdx = "None"), (state.clickedName = "None"))
        : ((state.clickedIdx =
            currentIdx >= state.tabComponents.length
              ? state.tabComponents.length - 1
              : currentIdx),
